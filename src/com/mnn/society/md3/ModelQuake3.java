...
public class ModelQuake3 {
	...
	public void load(String filePath, String fileModel) throws IOException {
		...
		if(!head.load(fileHeadModel)) {
			Logger.log(Logger.LOG_CLIENT, "CLIENT : LoaderMD3 : [Error]: unable to load the HEAD part from model \"" + fileModel + "\".");
			System.exit(0);
		}

		// Load the upper mesh (*_head.md3) and make sure it loaded properly
		if(!upper.load(fileUpperModel)) {
			Logger.log(Logger.LOG_CLIENT, "CLIENT : LoaderMD3 : [Error]: unable to load the UPPER part from model \"" + fileModel + "\".");
			System.exit(0);
		}

		// Load the lower mesh (*_lower.md3) and make sure it loaded properly
		if(!lower.load(fileLowerModel)) {
			Logger.log(Logger.LOG_CLIENT, "CLIENT : LoaderMD3 : [Error]: unable to load the LOWER part from model \"" + fileModel + "\".");
			System.exit(0);
		}

		// Load the lower skin (*_upper.skin) and make sure it loaded properly
		if(!lower.loadSkin(fileLowerSkin)) {
			Logger.log(Logger.LOG_CLIENT, "CLIENT : LoaderMD3 : [Error]: unable to load the LOWER part from model's skin \"" + fileModel + "\".");
			System.exit(0);
		}

		// Load the upper skin (*_upper.skin) and make sure it loaded properly
		if(!upper.loadSkin(fileUpperSkin)) {
			Logger.log(Logger.LOG_CLIENT, "CLIENT : LoaderMD3 : [Error]: unable to load the UPPER part from model's skin \"" + fileModel + "\".");
			System.exit(0);
		}

		// Load the head skin (*_head.skin) and make sure it loaded properly
		if(!head.loadSkin(fileHeadSkin)) {
			Logger.log(Logger.LOG_CLIENT, "CLIENT : LoaderMD3 : [Error]: unable to load the HEAD part from model's skin \"" + fileModel + "\".");
			System.exit(0);
		}
		...
		if(!loadAnimations(configFile)) {
			...
			Logger.log(Logger.LOG_CLIENT, "CLIENT : LoaderMD3 : [Error]: Unable to load the Animation Config File!");
			System.exit(0);
		}
	...
	public final void loadWeapon(String filePath, String fileModel) throws IOException {
		...
		if(!weapon.load(fileWeaponModel)) {
			Logger.log(Logger.LOG_CLIENT, "CLIENT : LoaderMD3 : [Error]: unable to load the weapon model \"" + fileModel + "\".");
			System.exit(0);
		}
		...
		if(!weapon.loadShader(fileWeaponShader)) {
			Logger.log(Logger.LOG_CLIENT, "CLIENT : LoaderMD3 : [Error]: unable to load the shader for weapon \"" + fileModel + "\".");
			System.exit(0);
		}
		...
		while((line = reader.readLine()) != null) {
			...
			// If the animation is for the legs, add it to the legs's list
				...
				if(torsoOffset == 0) {
					...
					torsoOffset = animations[Animations.LEGS_WALKCR.ordinal()].startFrame - 
							animations[Animations.TORSO_GESTURE.ordinal()].startFrame;
				}

				// Minus the offset from the legs animation start and end frame.
				animations[currentAnim].setStartFrame(animations[currentAnim].startFrame - torsoOffset);
				animations[currentAnim].setEndFrame(animations[currentAnim].endFrame - torsoOffset);

				// Add the animation to the list of leg animations
				lower.addAnimations(animations[currentAnim]);
			}
			else if (animName.startsWith("WEAP")) {
				this.weapon.addAnimations(animations[currentAnim]);
			}
	...
	public final void linkModel(ModelMD3 model, ModelMD3 link, String tagName) {
		...
		for(int i = 0; i < num; i++) {
			...
			if((tag = model.getTags(i).nameTag).equalsIgnoreCase(tagName)) {
				// Link the model's link index to the link (or model/mesh) and return
				Logger.log(Logger.LOG_CLIENT, "CLIENT : linkModel : tag '"+tag+"' matches '"+tagName+"'");
				model.setLinks(link, i);
				return;
			}
			Logger.log(Logger.LOG_CLIENT, "CLIENT : linkModel : tag '"+tag+"' not matching '"+tagName+"'");
		}
	}
	...
	private Quaternion qQuat = new Quaternion();
	private Quaternion qNextQuat = new Quaternion();
	private Quaternion qInterpolatedQuat = new Quaternion();
	private float[] pMatrix;
	private float[] pNextMatrix;
	...
	private void updateModel(ModelMD3 pModel) {
		...
		try {
			AnimationInfo pAnim = pModel.animations.get(pModel.currentAnim);

			// If there is any animations for this model
			if(pModel.animations.size() > 0) {
			...
			if(pModel.getNextFrame() == 0) { 
				if (pModel.nextAnimation.equalsIgnoreCase(""))
					pModel.setNextFrame(startFrame);
				else {
					this.setTorsoAnimation(pModel.nextAnimation);
					pModel.nextAnimation = "";
				}
			}
	...
	public void setNextAnimation (String nextAnimation) {
		this.upper.nextAnimation = nextAnimation;
	}
	...
	public void setTorsoAnimation(String strAnimation) {
		...
		for(int i = 0; i < upper.animations.size(); i++) {
			...
			if( upper.getAnimations(i).getAnimName().equalsIgnoreCase(strAnimation) ) {
				...
				upper.setCurrentFrame(upper.getAnimations(upper.getCurrentAnim()).startFrame);
				break;
			}
		}
	}
	...
	public void setLegsAnimation(String strAnimation) {
		// Go through all of the animations in this model
		for(int i = 0; i < lower.animations.size(); i++) {
			// If the animation name passed in is the same as the current animation's name
			if( lower.getAnimations(i).getAnimName().equalsIgnoreCase(strAnimation)) {
				// Set the legs animation to the current animation we just found and return
				lower.setCurrentAnim(i);
				lower.setCurrentFrame(lower.getAnimations(lower.getCurrentAnim()).startFrame);
				break;
			}
		}
	}
