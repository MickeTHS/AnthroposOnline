...
public class TextureLoader {
	...
	public TextureLoader() {
		...
		texBuffer = ByteBuffer.allocateDirect(4 * 100)
	 ...
	 */
	public Texture getTexture(String resourceName, int x, int y,
			boolean mipmap, boolean useAnisotropicFilter) throws IOException {
	...
	public Texture getTexture(String resourceName, int target,
			int dstPixelFormat, int x, int y, boolean mipmap,
			boolean useAnisotropicFilter) throws IOException {
		...
		if (!extension.equalsIgnoreCase("tga")) {
			...
			textureBuffer = TGALoader.loadImage(new BufferedInputStream(
					getBufferedInputStream(resourceName)));
		...
		if (!mipmap) {
			...
			if (cCap.GL_EXT_texture_compression_s3tc) {
				// If so, generate the texture using the DXT5 compression method
				// (best copression).
				// Notice that we now use GL_RGBA to generate the texture.
				Logger.log(Logger.LOG_CLIENT, "GL_COMPRESSED_RGBA_S3TC_DXT5_EXT available !");
				pixelFormat = EXTTextureCompressionS3TC.GL_COMPRESSED_RGBA_S3TC_DXT5_EXT;
			} else {
				// If extension isn't available (should only be the case on old
				// video cards), generate
				// an uncompressed texture.
				Logger.log(Logger.LOG_CLIENT, "GL_COMPRESSED_RGBA_S3TC_DXT5_EXT unavailable...");
				// produce a texture from the byte buffer
				pixelFormat = dstPixelFormat;
			}

			GL11.glTexImage2D(target, 0, pixelFormat, get2Fold(width),
			...
			try {
				...
				GLU.gluBuild2DMipmaps(target, dstPixelFormat,
						get2Fold(texWidth), get2Fold(texHeight),
	...
	private ByteBuffer convertImageData(BufferedImage bufferedImage,
		...
		transform.scale(x, y);
		transform.translate((x - 1) * bufferedImage.getWidth() / 2, (y - 1)
	...
	private BufferedInputStream getBufferedInputStream(String ref)
			throws IOException {

		File file = new File(ref);
		Logger.log(Logger.LOG_CLIENT, file.getAbsolutePath());
		InputStream is = null;
