        ...
        public boolean importMD3(ModelMD3 model, String fileName) {
        	try {
                ...
                if(!ID.equalsIgnoreCase("IDP3") || header.version != 15) {
                	Logger.log(Logger.LOG_CLIENT, "CLIENT : LoaderMD3 : [Error]: file " + fileName + " version is not valid.");
                	System.exit(0);
                }
            	...
            	Logger.log(Logger.LOG_CLIENT, "CLIENT : LoaderMD3 : [Error]: can't read " + fileName + " correctly.");
            	System.exit(0);
            }
        ...
        private void readMD3Data(ModelMD3 model) throws Exception {
            ...
            for (int j = 0; j < header.numMeshes; j++) {
                ...
                for(i = 0; i < meshHeader.numMeshFrames * meshHeader.numVertices; i++) {
                	...
                	if (this.min == null) {
                		this.min = new float [] { vertices[0].vertex[0], vertices[0].vertex[1], vertices[0].vertex[2] };
                        this.max = new float [] { vertices[0].vertex[0], vertices[0].vertex[1], vertices[0].vertex[2] };
                	}
                	else {
	                	for (int n = 0; n < 3; n++) {
	                		if (this.min[n] > vertices[i].vertex[n]) this.min[n] = vertices[i].vertex[n];
	                		if (this.max[n] < vertices[i].vertex[n]) this.max[n] = vertices[i].vertex[n];
	                	}
                	}
                }
